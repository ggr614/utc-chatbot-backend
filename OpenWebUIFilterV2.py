"""
title: RAG Helpdesk Filter V2 (Query Sandwich)
author: David Wood
version: 1.0.0
date: 2026-03-06
description: Filter that augments any model with RAG context using a "query sandwich"
             prompt structure. The user's query appears before the system prompt AND after
             the retrieved context, combating the "lost in the middle" problem.
             Default behavior performs hybrid search (BM25 + vector). Use !f for follow-up
             mode (LLM-only, no RAG). Use !help for command help.
license: MIT
...
class Filter:
    """
    RAG Helpdesk Filter V2 (Query Sandwich) - Augments any Open WebUI model with
    knowledge base context using a query-sandwich prompt structure.

    This filter:
    1. Intercepts the user's message (inlet)
    2. Searches the RAG Helpdesk API for relevant documents (default)
    3. Injects the query BEFORE the system prompt AND AFTER the retrieved context
    4. Passes the augmented request to the selected model

    The "query sandwich" structure combats the "lost in the middle" problem by
    placing the user's question at both ends of the injected content.
    ...
    def __init__(self):
        ...
        self.name = "RAG Helpdesk Filter V2"
...
*Need more help? Contact the development team or check the system documentation.*"""
    ...
    def _search_documents(
        ...
        if self.DEBUG_MODE:
            print("[RAG Filter V2] Using standard hybrid search endpoint")
        ...
        if self.DEBUG_MODE:
            print(f"[RAG Filter V2] ========== API REQUEST ==========")
            print(f"[RAG Filter V2] URL: {url}")
            print(
                f"[RAG Filter V2] Headers: {json.dumps({k: v[:20] + '...' if k == 'X-API-Key' and len(v) > 20 else v for k, v in headers.items()})}"
            )
            print(f"[RAG Filter V2] Payload: {json.dumps(payload, indent=2)}")
            print(f"[RAG Filter V2] Timeout: {self.REQUEST_TIMEOUT}s")
        ...
        try:
            ...
            if self.DEBUG_MODE:
                print(f"[RAG Filter V2] ========== API RESPONSE ==========")
                print(f"[RAG Filter V2] Status Code: {response.status_code}")
                print(f"[RAG Filter V2] Response Headers: {dict(response.headers)}")
                print(
                    f"[RAG Filter V2] Response Body (first 500 chars): {response.text[:500]}"
                )
            ...
            if self.DEBUG_MODE:
                print(f"[RAG Filter V2] ========== CONNECTION ERROR ==========")
                print(f"[RAG Filter V2] Could not connect to: {url}")
                print(f"[RAG Filter V2] Error type: {type(e).__name__}")
                print(f"[RAG Filter V2] Error details: {str(e)}")
                print(f"[RAG Filter V2] Traceback:\n{traceback.format_exc()}")
            raise

        except requests.exceptions.Timeout as e:
            if self.DEBUG_MODE:
                print(f"[RAG Filter V2] ========== TIMEOUT ERROR ==========")
                print(
                    f"[RAG Filter V2] Request timed out after {self.REQUEST_TIMEOUT}s"
                )
                print(f"[RAG Filter V2] URL: {url}")
            raise
            ...
            if self.DEBUG_MODE:
                print(f"[RAG Filter V2] ========== HTTP ERROR ==========")
                print(f"[RAG Filter V2] Status Code: {e.response.status_code}")
                print(f"[RAG Filter V2] Response Body: {e.response.text}")
                print(f"[RAG Filter V2] Request URL: {url}")
            raise

        except Exception as e:
            if self.DEBUG_MODE:
                print(f"[RAG Filter V2] ========== UNEXPECTED ERROR ==========")
                print(f"[RAG Filter V2] Error type: {type(e).__name__}")
                print(f"[RAG Filter V2] Error message: {str(e)}")
                print(f"[RAG Filter V2] Traceback:\n{traceback.format_exc()}")
            raise
    ...
    def _format_context(self, search_response: Dict[str, Any]) -> str:
        ...
        for i, chunk in enumerate(results, 1):
            ...
            if total_chars + len(doc_entry) > max_chars:
                if self.DEBUG_MODE:
                    print(f"[RAG Filter V2] Truncating at doc {i} due to token limit")
                break
    ...
    def _inject_context_into_messages(
        ...
        """Inject RAG context into the message list using query-sandwich structure.
        ...
        This places the query before the instructions (priming) and after the
        context (reminder), combating the "lost in the middle" problem.

        Args:
            messages: The conversation message list
            context: Formatted document context string
            query: The user's search query to sandwich around the content
        ...
        # Build the query-sandwich system message
        rag_instruction = f"""User question: {query}
...
User question: {query}"""
        ...
        for msg in messages:
            if msg.get("role") == "system":
                # Replace existing system prompt with sandwich RAG prompt
                new_messages.append(
                    {
                        "role": "system",
    ...
    async def inlet(
        ...
        3. Injects retrieved context with query-sandwich structure
        ...
        print(f"[RAG Filter V2] ========== INLET TRIGGERED ==========")
        print(f"[RAG Filter V2] Debug Mode: {self.DEBUG_MODE}")
        print(f"[RAG Filter V2] RAG API URL: {self.valves.RAG_API_BASE_URL}")
        print(
            f"[RAG Filter V2] API Key configured: {'Yes' if self.valves.RAG_API_KEY else 'NO - MISSING!'}"
        )

        if self.DEBUG_MODE:
            print(f"[RAG Filter V2] User: {__user__}")
            print(f"[RAG Filter V2] Body keys: {body.keys()}")
        ...
        if not self.valves.RAG_API_KEY:
            ...
            print(f"[RAG Filter V2] ERROR: {error_msg}")
        ...
        if not user_query:
            print("[RAG Filter V2] No user query found in messages, passing through")
            if self.DEBUG_MODE:
                print(f"[RAG Filter V2] Messages: {json.dumps(messages, indent=2)}")
            return body

        print(
            f"[RAG Filter V2] Query: {user_query[:100]}{'...' if len(user_query) > 100 else ''}"
        )
        ...
        print(f"[RAG Filter V2] Command Mode: {self._command_mode}")

        # Handle !help command - return help text immediately
        if command == "help":
            print("[RAG Filter V2] Help command detected - returning help text")
            ...
            print(f"[RAG Filter V2] Help command completed in {elapsed}ms")
            print("[RAG Filter V2] =====================================")
        ...
        if command == "follow_up":
            print("[RAG Filter V2] Follow-up mode - bypassing RAG retrieval")
            if __event_emitter__:
                await __event_emitter__(
                    {
                        "type": "status",
            ...
            print("[RAG Filter V2] Injected no-RAG system prompt")
            return body

        # Default: use cleaned query for hybrid search
        user_query = cleaned_query
        print(
            f"[RAG Filter V2] Cleaned Query: {user_query[:100]}{'...' if len(user_query) > 100 else ''}"
        )
        ...
        try:
            ...
            if api_system_prompts and search_response.get("results"):
                ...
                if resolved_system_prompt:
                    print(
                        f"[RAG Filter V2] Using API system prompt from article {top_article_id[:8]}... ({len(resolved_system_prompt)} chars)"
                    )
                else:
                    print(
                        "[RAG Filter V2] Top article not in system_prompts dict, using hardcoded fallback"
                    )
            elif self.DEBUG_MODE:
                print(
                    "[RAG Filter V2] No system_prompts in API metadata, using hardcoded fallback"
                )
            ...
            if query_log_id and self.ENABLE_LLM_RESPONSE_LOGGING:
                ...
                if self.DEBUG_MODE:
                    print(
                        f"[RAG Filter V2] Stored query_log_id {query_log_id} for response logging"
                    )

            print(f"[RAG Filter V2] SUCCESS: Retrieved {num_docs} documents")
            if self.DEBUG_MODE:
                latency = search_response.get("latency_ms", "?")
                print(f"[RAG Filter V2] API Latency: {latency}ms")
                print(f"[RAG Filter V2] Context length: {len(context)} chars")
        ...
        # Handle errors
        if error_msg:
            print(f"[RAG Filter V2] ========== ERROR ==========")
            print(f"[RAG Filter V2] Error: {error_msg}")
            print(f"[RAG Filter V2] Detail: {error_detail}")
            ...
            print(
                "[RAG Filter V2] Graceful degradation enabled, continuing without RAG context"
            )
            return body

        # Inject context with query sandwich if we have any
        if context:
            ...
            print(
                f"[RAG Filter V2] Injected query-sandwich context from {num_docs} documents"
            )
        else:
            if __event_emitter__:
                await __event_emitter__(
            ...
            print("[RAG Filter V2] No matching documents found")

        elapsed = int((time.time() - start_time) * 1000)
        print(f"[RAG Filter V2] Inlet completed in {elapsed}ms")
        print("[RAG Filter V2] =====================================")
        ...
        try:
            ...
            if self.ENABLE_LLM_RESPONSE_LOGGING:
                ...
                if query_log_id:
                    ...
                    print(
                        "[RAG Filter V2] No query_log_id found, skipping response logging"
                    )
            elif self.DEBUG_MODE:
                print("[RAG Filter V2] LLM response logging disabled")
    ...
    def _log_llm_response(self, body: Dict[str, Any], query_log_id: int) -> None:
        ...
        try:
            ...
            if not messages:
                if self.DEBUG_MODE:
                    print("[RAG Filter V2] No messages in response body")
                return
            ...
            if last_message.get("role") != "assistant":
                if self.DEBUG_MODE:
                    print(
                        f"[RAG Filter V2] Last message not from assistant: {last_message.get('role')}"
                    )
                return
            ...
            if not response_text:
                if self.DEBUG_MODE:
                    print("[RAG Filter V2] Empty response text")
                return
            ...
            if not response_text.strip():
                if self.DEBUG_MODE:
                    print("[RAG Filter V2] Response text is empty after extraction")
                return

            print(f"[RAG Filter V2] Logging LLM response for query_log_id {query_log_id}")
            print(f"[RAG Filter V2] Response length: {len(response_text)} chars")
            ...
            if self.DEBUG_MODE:
                print("[RAG Filter V2] ========== LLM RESPONSE LOGGING ==========")
                print(f"[RAG Filter V2] URL: {url}")
                print(f"[RAG Filter V2] Payload: {json.dumps(payload, indent=2)[:500]}...")
            ...
            try:
                ...
                print(
                    f"[RAG Filter V2] LLM response logged successfully: ID {result.get('id')}"
                )
                ...
                if e.response.status_code == 409:
                    ...
                    print(
                        f"[RAG Filter V2] LLM response already logged for query_log_id {query_log_id}"
                    )
                else:
                    print(
                        f"[RAG Filter V2] Failed to log LLM response (HTTP {e.response.status_code}): {e.response.text[:200]}"
                    )

            except Exception as e:
                # Don't fail the outlet on logging errors
                print(
                    f"[RAG Filter V2] Failed to log LLM response: {type(e).__name__}: {str(e)}"
                )
                if self.DEBUG_MODE:
                    print(f"[RAG Filter V2] Traceback:\n{traceback.format_exc()}")
            ...
            print(
                f"[RAG Filter V2] Error in outlet LLM logging: {type(e).__name__}: {str(e)}"
            )
            if self.DEBUG_MODE:
                print(f"[RAG Filter V2] Traceback:\n{traceback.format_exc()}")
