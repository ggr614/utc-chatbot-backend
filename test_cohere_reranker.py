...
def test_reranker_connection():
    ...
    try:
        ...
        logger.info(
            f"  - Access Key ID: {settings.ACCESS_KEY_ID.get_secret_value()[:8]}..."
        )
    ...
    try:
        ...
        if error_code == "AccessDeniedException":
            logger.error(
                "  Hint: Check that your AWS credentials have bedrock:InvokeModel permissions"
            )
    ...
    try:
        ...
        for i, result in enumerate(results, 1):
            ...
            logger.info(
                f"  Text: {doc_text[:100]}{'...' if len(doc_text) > 100 else ''}"
            )
            logger.info("")
